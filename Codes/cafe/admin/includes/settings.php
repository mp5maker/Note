...
if($db->num() > 1):
	$fetch = $db->fetch();
	for($i = 0; $i < $db->num(); $i++):
		$navigation_id[$fetch[$i][0]] = $fetch[$i][0];
	endfor;
endif;
...
if($db->num() > 1):
	$fetch = $db->fetch();
	for($i = 0; $i < $db->num(); $i++):
		$chart_reason[$fetch[$i][0]] = $fetch[$i][1];
	endfor;
endif;
...
if($db->num() > 1):
	$fetch = $db->fetch();
	for($i = 0; $i < $db->num(); $i++):
		$promotion_name[$fetch[$i][0]] =  $fetch[$i][1];
	endfor;
endif;
...
if($db->num() >= 1):
	$fetch = $db->fetch();
	for($i = 0; $i < $db->num(); $i++):
		$submenu_name[$fetch[$i][0]] = $fetch[$i][1];
	endfor;
endif;
...
if(isset($_GET['admin'])):
	if($_GET['admin'] == true):
		$db->table('admin');
		$db->select(['id', 'email', 'date_created']);
		if($db->num() >= 1):
			$fetch = $db->fetch();
			for($i = 0; $i < $db->num(); $i++):
				$admin[] = ['id' => $fetch[$i][0], 'email' => $fetch[$i][1], 'date_created' => $fetch[$i][2]];
			endfor;
...
if(isset($_GET['email'])):
	if($_GET['email'] == true):
		$db->table('email');
		$db->select(['id', 'name', 'email', 'gender', 'question', 'description']);
		if($db->num() >= 1):
			$fetch = $db->fetch();
			for($i = 0; $i < $db->num(); $i++):
				$email[] = ['id' => $fetch[$i][0], 'name' => $fetch[$i][1], 'email' => $fetch[$i][2], 'gender' => $fetch[$i][3], 'question' => $fetch[$i][4], 'description' => $fetch[$i][5]];
			endfor;
